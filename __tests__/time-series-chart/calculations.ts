...
describe("time-series-chart calculations test", () => {
  ...
  test('evenly distributed time series is placed into buckets properly', () => {
  ...
  test('time series is transformed to no empty buckets', () => {
  ...
  test('square (hourly) wave', () => {
    ...
    try {
      ...
      console.error(`[test-ctx] Run with minutes: ${minutes}, numberOfBuckets: ${numberOfBuckets}, division: ${division}`);
      throw error;
    }
  });
  ...
  test('not-evenly distributed series gets transformed to buckets under specific filter conditions - test B', () => {
    try {
      ...
      console.error('[test-ctx] notEvenlyDistributedSeries:', JSON.stringify(notEvenlyDistributedSeries));
      throw error;
    }
  });
